...
import FilterSidebar from "../components/common/FilterSidebar";
import useProductFilters from "../hooks/useProductFilters";
import { ChevronRight, SlidersHorizontal, Filter, X } from "lucide-react";
...
const Products = () => {
  ...
  // 1. URL se params nikalna (Keywords, Category, Sorting)
  const keyword = searchParams.get("keyword") || "";
  const categoryId = searchParams.get("category") || ""; // Backend 'categoryId' expect kar raha hai
  const sortBy = searchParams.get("sortBy") || "newest";

  // const filters = { keyword, category: categoryId, sortBy };
     const filters = {
    keyword: searchParams.get("keyword") || "",
    category: searchParams.get("category") || "",
    sortBy: searchParams.get("sortBy") || "",
  ...
  // 2. FETCH DATA: Jab bhi URL ka keyword ya category badlega, ye fetch trigger hoga
  // useEffect(() => {
  //   const fetchData = async () => {
  //     setLoading(true);
  //     try {
  ...
  //   fetchData();
  // }, [keyword, categoryId]); // 🔗 dependency array: keyword ya categoryId badalne par refresh hoga

  // upar jo fetch wo backend search filter ke sath or ye fronted ke sath hi hoga
useEffect(() => {
  const fetchData = async () => {
    setLoading(true);
    try {
      // Khali string bhej rahe hain kyunki backend ab filter nahi kar raha
      const { data } = await getProducts(""); 
      setProducts(data || []);
      
      // Categories set karne wala logic same rahega
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  fetchData();
}, []); // Dependency array empty rakhein taaki sirf ek baar call ho

  // 3. Frontend Sorting (Kyunki humne sorting backend par nahi likhi abhi)
  const filteredProducts = useProductFilters(products, { sortBy });
            ...
            <div className="fixed inset-0 z-100 lg:hidden">
              ...
              <div className="absolute right-0 top-0 h-full w-[85%] max-w-sm bg-white p-8 shadow-2xl animate-in slide-in-from-right duration-500 overflow-y-auto rounded-l-[3rem]">
                <div className="flex justify-between items-center mb-10">
                  ...
                  <button onClick={() => setIsMobileFilterOpen(false)} className="p-3 bg-gray-100 rounded-2xl hover:bg-black hover:text-white transition-all">
                    <X size={20} />
                  </button>
                </div>
                <FilterSidebar
